...
@section('content')
    ...
    {{-- Hero Section avec la dernière publication à la une --}}
    @if ($latestFeatured)
        <section class="hero-section">
            <div class="hero-bg"
                @if ($latestFeatured->featured_image)
                    @php
                        $extension = pathinfo($latestFeatured->featured_image, PATHINFO_EXTENSION);
                        $isVideo = in_array(strtolower($extension), ['mp4', 'mov', 'avi', 'webm']);
                    @endphp
                    @if ($isVideo)
                        style="background: #000;">
                        <video autoplay muted loop playsinline class="absolute inset-0 w-full h-full object-cover">
                            <source src="{{ asset('storage/' . $latestFeatured->featured_image) }}" type="video/{{ $extension }}">
                        </video>
                        <!-- Play Button miniature -->
                        <div class="video-overlay">
                            <div class="play-button" style="width: 120px; height: 120px;">
                                <svg fill="currentColor" viewBox="0 0 20 20" style="width: 56px; height: 56px;">
                                    <path d="M6.3 2.841A1.5 1.5 0 004 4.11V15.89a1.5 1.5 0 002.3 1.269l9.344-5.89a1.5 1.5 0 000-2.538L6.3 2.84z" />
                                </svg>
                            </div>
                        </div>
                    @else
                        style="background-image: url('{{ asset('storage/' . $latestFeatured->featured_image) }}');"
                    @endif
                @endif>

                <a href="{{ route('publication.show', $latestFeatured->slug) }}" class="block h-full w-full">
                    ...
                    <div class="hero-content">
                        <!-- Badge "Événement du jour" mis en avant -->
                        <div class="mb-4">
                            {{-- <span class="inline-flex items-center gap-2 bg-gradient-to-b from-blue-600 to-orange-500 text-white px-6 py-3 rounded-full font-black text-base sm:text-lg uppercase tracking-wider shadow-2xl animate-pulse"> --}}
                                <span class="inline-flex items-center gap-2 bg-gradient-to-r from-blue-600 to-blue-800 text-white px-6 py-3 rounded-full font-black text-base sm:text-lg uppercase tracking-wider shadow-2xl animate-pulse">
                        ...
                        <!-- Rubrique -->
                        <span class="hero-category">{{ $latestFeatured->rubrique->name }}</span>

                        <!-- Date -->
                        <div class="hero-date">
                            {{ $latestFeatured->published_at->locale('fr')->isoFormat('DD MMMM YYYY - HH[H]mm') }}
                        </div>

                        <!-- Titre -->
                        <h1 class="hero-title">{{ $latestFeatured->title }}</h1>
                    </div>
                </a>
            </div>
        </section>
                ...
                <div class="mb-6 overflow-x-auto pb-2 filter-buttons">
                    <div class="flex gap-2 min-w-max">
                        @foreach ($metaKeywords as $item)
                            <a href="{{ route('publication.show', $item->slug ?? '#') }}"
                        ...
                        @foreach ($recentArticles as $article)
                            ...
                            @endif --}}
                                    ...
                                    <div class="p-5">
                                        ...
                                        <h3 class="text-lg font-bold mb-3 leading-tight line-clamp-1">
                                            {{ $article->title }}
                                        </h3>
                            ...
                            @foreach ($recentArticles->take(30) as $article)
                                <article class="p-4 hover:bg-gray-100 bg-white transition cursor-pointer rounded">
                                    <a href="{{ route('publication.show', $article->slug) }}" class="block">
                                        ...
                                        <!-- Titre -->
                                        <h4
                                            class="font-bold text-sm leading-tight mb-3 line-clamp-1 hover:text-blue-600 transition">
                                            {{ $article->title }}
                                        </h4>
                                        <!-- Image + Texte -->
                        ...
                        <div class="flex gap-4 py-4 border-b last:border-0 hover:bg-gray-50 cursor-pointer transition">
                            ...
                            <div class="flex-1 min-w-0">
                                <a href="{{ route('publication.show', $popular->slug) }}">
                                    <h4 class="font-semibold text-sm leading-tight mb-2 hover:text-blue-600 line-clamp-1">
                                        {{ $popular->title }}
                                    </h4>
                    ...
                    @foreach ($politiqueArticles as $article)
                        <article
                            ...
                            <a href="{{ route('publication.show', $article->slug) }}">
                                ...
                                <div class="p-5">
                                    <h3 class="font-bold text-base leading-tight mb-3 line-clamp-1">
                                        {{ $article->title }}
                                    </h3>
                                    <span class="text-xs text-gray-500 flex">
                                        <svg class="w-4 h-4 mx-2" fill="none" stroke="currentColor"
