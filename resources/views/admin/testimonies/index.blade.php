...
@push('scripts')
    <script>
        ...
        function displayTestimonies() {
            ...
            tbody.innerHTML = testimonies.map(testimony => `
                <tr class="hover:bg-gray-50 ${selectedIds.has(testimony.id) ? 'bg-blue-50' : ''}">
                    <td class="px-6 py-4">
                        <input type="checkbox"
                            class="testimony-checkbox rounded"
                            value="${testimony.id}"
                            ${selectedIds.has(testimony.id) ? 'checked' : ''}
                    ...
                    <td class="px-6 py-4 text-sm text-gray-900">${testimony.views}</td>
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <div class="text-sm text-gray-900">${testimony.views}</div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="mt-1 text-sm text-gray-900 whitespace-pre-wrap">${escapeHtml(testimony.description)}</div>
                            </div>
                            ...
                            ${testimony.media_files && testimony.media_files.length > 0 ? `
                                <div>
                                    <label class="text-sm font-medium text-gray-500 mb-2 block">Médias (${testimony.media_files.length})</label>
        ...
        async function deleteMedia(testimonyId, mediaPath) {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    showDetails(testimonyId); // Recharger les détails
                } else {
                    showNotification(data.message || 'Erreur lors de la suppression', 'error');
                }
            } catch (error) {
        ...
        function updateStats(stats) {
            ...
            document.getElementById('stat-views').textContent = stats.total_views;
        }
