...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm p-6">
        ...
        <h2 class="text-2xl font-bold text-gray-900 mb-6">Modifier {{ $teamMember->full_name }}</h2>

        <form method="POST" action="{{ route('admin.team.update', $teamMember) }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="prenom" name="prenom" required
                        value="{{ old('prenom', $teamMember->prenom) }}"
            ...
            <div>
                ...
                <input type="text" id="poste" name="poste" required
                    value="{{ old('poste', $teamMember->poste) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="telephone" name="telephone"
                        value="{{ old('telephone', $teamMember->telephone) }}"
            ...
            @if($teamMember->photo)
                <div>
                    ...
                    <div class="flex items-center gap-4">
                        <img src="{{ $teamMember->photo_url }}" alt="{{ $teamMember->full_name }}"
                            class="w-24 h-24 rounded-full object-cover border-2 border-gray-200">
                        <form method="POST" action="{{ route('admin.team.delete-photo', $teamMember) }}"
                            onsubmit="return confirm('Supprimer cette photo ?')" class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
                                class="text-red-600 hover:text-red-800 font-medium inline-flex items-center gap-2">
            ...
            <div>
                <label for="photo" class="block text-sm font-medium text-gray-700 mb-2">
                    {{ $teamMember->photo ? 'Remplacer la photo' : 'Photo (optionnelle)' }}
                </label>
            ...
            <div>
                ...
                <textarea id="bio" name="bio" rows="4"
                    ...
                    placeholder="Quelques mots sur ce membre...">{{ old('bio', $teamMember->bio) }}</textarea>
                @error('bio')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" id="ordre" name="ordre" min="0"
                    value="{{ old('ordre', $teamMember->ordre) }}"
            ...
            <div class="flex items-start gap-3">
                <input type="checkbox" id="is_visible" name="is_visible" value="1"
                    {{ old('is_visible', $teamMember->is_visible) ? 'checked' : '' }}
